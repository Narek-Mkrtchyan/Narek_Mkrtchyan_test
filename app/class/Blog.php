...
class Blog
{
	...
	public function EditBlog($arr_post)
	{
		$info = ''; 
		foreach($arr_post as $key => $value)
		{			
		...
		$query = "UPDATE ".$this->_mTable."  SET $info where id = ?";  //die($query);
		$sth = $this->_mDbh->prepare($query);
		$sth->execute(array(intval($arr_post['id'])));
	...
	public function GetBlogs($from_date = '', $to_date  = '')
	{
		...
		$sth = $this->_mDbh->prepare($query);  //echo($query);
